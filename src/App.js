...
const App = () => {
  ...
  const voteGif = async (gift_link) => {
    ...
    try {
      ...
      await program.rpc.updateItem(gift_link, {
        accounts: {
          baseAccount: baseAccount.publicKey,
          user: provider.wallet.publicKey,
        },
      });
      console.log("GIF successfully voted", gift_link);
          ...
          <div className="gif-grid">
            ...
            {gifList.map((item, index) => (
              <div className="gif-item" key={index}>
                ...
                <div>
                  <button onClick={() => voteGif(item.gifLink)}>Vote</button>
  ...
  const getGifList = useCallback(async () => {
    try {
      const provider = getProvider();
      const program = new Program(idl, programID, provider);
      console.log(program.account.baseAccount.fetch);
